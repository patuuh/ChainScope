...
class TestIndexing:
    ...
    def test_audit_skips_neutral_function_metadata_parses(self, tmp_db, monkeypatch):
        ...
        nested_risk_metadata = json.dumps({"nested": {"reentrancy_risk": True}, "large": ["x"] * 20})
        db.insert_node(
            id=entry_id,
            label="entry",
            type="function",
        ...
        db.insert_node(
            id="Vault.sol::Vault.nestedRisk()",
            label="nestedRisk",
            type="function",
            visibility="external",
            file="Vault.sol",
            metadata=nested_risk_metadata,
        )
        for i in range(30):
            db.insert_node(
        ...
        assert audit["source_context_summary"] == {"production": 33}
        ...
        assert nested_risk_metadata not in parsed
